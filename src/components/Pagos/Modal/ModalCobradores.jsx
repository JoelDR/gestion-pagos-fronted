...
export default function ModalCobradores({ state, setState, setCobrador }) {
  let tableHead = ['RUC', 'Entidad', 'Servicio', 'Dirección', 'Acción']
  const [listaCobradores, setListaCobradores] = useState(null);
          ...
          <div className='fixed flex p-8 justify-center h-screen bg-gray-800 bg-opacity-50 top-0 left-0 w-screen'>
            <div className='flex flex-col w-3/4 mt-3 bg-white opacity-100 rounded-lg px-8 py-8'>
              <header className="flex justify-between mb-3">
                <h3 className="text-lg font-bold text-indigo-600">Lista de Cobradores</h3>
                <button 
              ...
              <table className="divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {
                    listaCobradores && filtrarCobradores().map(cobrador => (
                      <tr key={cobrador._id}>
                        <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm text-gray-500">{cobrador.ruc}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-900">{cobrador.nombre}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900">{cobrador.servicio}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                          <div className="text-sm text-gray-900">{cobrador.direccion}</div>
                        </td>
              ...
              {
                listaCobradores && listaCobradores.length > 5 && (
