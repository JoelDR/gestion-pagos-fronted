...
export default function AgregarPago() {
  ...
  useEffect(() => {
    axios.get(APIURL+'pagosfull').then(res => {
      const pago = res.data.pagosfull.filter( pago => pago._id === params.id);
      setPago(pago[0]);
    });
  }, [APIURL]);
    ...
    <div className="mt-10 sm:mt-0">
      {
        pago && (
          <Formik
            ...
            initialValues={{ 
              deudor: pago.deudors[0].nombre,
              cobrador: pago.cobradors[0].nombre,
              fecha: pago.fecha.substr(0,10),
              total: pago.total,
            }}
            validate={(valores) => {
              let errores = {};
